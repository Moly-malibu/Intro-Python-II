...
if __name__=="__main__": 
    ...
    while True:
        print('', '---', '', sep='\n')
        print(player.adventure_room.name.upper())
        for A in wrap(player.adventure_room.description, 100):
            print (A)
# * Prints the current description (the textwrap module might be useful here).

        if player.adventure_room.items:
            print('\n Items: ')
            for item in player.adventure_room.items:
                print(f' {item.name}: {item.description}') 
        adventure_desc = input('> ').lower().split(' ')
        verb = adventure_desc[0]
 ...
 # Print an error message if the movement isn't allowed.
            ...
            if len(adventure_desc) < 2:
                print(f'Enter an object to {verb}.')
                input('\n --Enter any key to continue--')
                continue
            object = adventure_desc[1]
            if verb == 'drop':
            ...
            if not drop_item:
                ...
                player.adventure_room.items.append(
                    player.items.pop(idy)
                )
                drop_item.on_drop()
        elif verb in ['get', 'take']:
            take_time, take_idy = None, None
            for idy, item in enumerate(player.adventure_room.items):
                if object == item.name.lower():
                    take_idy = idy
                    take_item = item
            if not take_item:
                ...
                player.items.append(
                    player.adventure_room.items.pop(idy)
                )
                take_item.on_take()
        elif verb == 'inventory':
            if player.items:
